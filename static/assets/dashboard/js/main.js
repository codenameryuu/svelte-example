...
(function () {
  ...
  if (typeof Waves !== "undefined") {
    Waves.init();
    Waves.attach(".btn[class*='btn-']:not(.position-relative):not([class*='btn-outline-']):not([class*='btn-label-']):not([class*='btn-text-'])", [
      "waves-light",
    ]);
  ...
  window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change", () => {
    var storedTheme = window.Helpers.getStoredTheme();
  ...
  window.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelectorAll("[data-bs-theme-value]").forEach((toggle) => {
      toggle.addEventListener("click", () => {
        ...
        var semiDarkL = document.querySelector(".template-customizer-semiDark");
        if (semiDarkL) {
          if (theme === "dark") {
  ...
  return autocomplete({
    ...
    onStateChange({ state, setQuery }) {
      ...
      if (state.isOpen) {
        ...
        var cancelIcon = document.querySelector(".aa-DetachedCancelButton");
        if (cancelIcon) {
    ...
    render(args, root) {
      var { render, html, children, state } = args;

      // Initial Suggestions
      if (!state.query) {
        var initialSuggestions = html`
    ...
    getSources() {
      ...
      if (data.navigation) {
        ...
        var navigationSources = Object.keys(data.navigation)
          .filter((section) => section !== "files" && section !== "members")
          .map((section) => ({
            sourceId: `nav-${section}`,
            getItems({ query }) {
              var items = data.navigation[section];
        ...
        if (data.navigation.files) {
          sources.push({
            sourceId: "files",
            getItems({ query }) {
              var items = data.navigation.files;
              if (!query) return items;
              return items.filter((item) => item.name.toLowerCase().includes(query.toLowerCase()));
            },
            getItemUrl({ item }) {
        ...
        if (data.navigation.members) {
          sources.push({
            sourceId: "members",
            getItems({ query }) {
              var items = data.navigation.members;
              if (!query) return items;
              return items.filter((item) => item.name.toLowerCase().includes(query.toLowerCase()));
            },
            getItemUrl({ item }) {
