...
(function (root, factory) {
  ...
  function Dropify(element, options) {
    ...
    var defaults = {
      ...
      tpl: {
        ...
        linkButton: '<button type="button" class="dropify-link">{{ link }}</button>',
  ...
  Dropify.prototype.createElements = function () {
    ...
    if (this.isDisabled === false && this.settings.showRemove === true) {
      ...
      const defaultFile = this.input.data("default-file");

      if (defaultFile) {
        this.linkButton = $(this.settings.tpl.linkButton);
        this.linkButton.insertAfter(this.input);
        this.linkButton.on("click", this.linkElement);
      }
    }
  ...
  /**
   * Link element, events are available
   */
  Dropify.prototype.linkElement = function () {
    const defaultFile = this.input.attr("data-default-file");

    if (defaultFile) {
      var eventBefore = $.Event("dropify.beforeLink");
      this.input.trigger(eventBefore, [this]);

      if (eventBefore.result !== false) {
        window.open(defaultFile, "_blank");

        this.input.trigger($.Event("dropify.afterLink"), [this]);
      }
    }
  };
